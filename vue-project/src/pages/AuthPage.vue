...
<script setup>
import { ref } from 'vue';
import { useAppStore } from '../app/providers/store.js';
...
//         return this.id ? mainJson.users.find(user => user.email === login && user.password === pass).id : null
//     },
//     User_CheckRole(id){
...
//     login() {
//       const iduser = Login(this.UserLogin, this.UserPassword);
//       const isStudent = User_CheckRole(iduser)
//       this.Password_Is_Valid = result;
//       if (result || this.isEzAuth) {
//         console.log(`Успешная авторизация! Роль пользователя: ${User.Get_User_Name()}`);
